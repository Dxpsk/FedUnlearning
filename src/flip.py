...
def trigger_fast_train(model, data_iterator, start_epoch, agent_name_key, args = None):
    device = args.device
    model.train()
    model.to(device)
    ...
    trigger = Trigger(
        model, steps=trigger_steps, attack_succ_threshold=0.85, num_classes=num_classes,args=args
    )
    trigger_combo = TriggerCombo(model, attack_succ_threshold=0.85, steps=trigger_steps, args=args)
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch in my_dataloader:
            ...
            if len(label_grt_min) == 0:
                ...
                continue
            # trigger stamping
            # logging.info(f"step: {step}, max_warmup_steps: {max_warmup_steps}")
                ...
                if WARMUP:
                    ...
                    if np.random.rand() < 0.3:  # scheduler
                        ...
                        if np.isnan(diff_sum[source, target]):
                            # logging.info("encounter nan during selection!")
                            exit()
                    trigger_steps = 200

                key = f"{source}-{target}" if source < target else f"{target}-{source}"
                # logging.info(f"source: {source}, target: {target}, key: {key}")
