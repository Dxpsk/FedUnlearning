...
def trigger_fast_train(model, data_iterator, start_epoch, agent_name_key, args = None):
    ...
    # logging.info(f"parameters: portion: {portion}")

    # set up trigger generation
    trigger = Trigger(
        model, steps=trigger_steps, attack_succ_threshold=0.90, num_classes=num_classes, args=args
    )
    trigger_combo = TriggerCombo(model, steps=trigger_steps, args=args)
